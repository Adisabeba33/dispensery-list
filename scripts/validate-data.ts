...
import { brandKeyOf, menuIdentity } from './menu-render.mjs';
...
const validateFlowerListings = (FILE: string) => {
  ...
  raw.forEach((record) => {
    ...
    const key = menuIdentity(r.sources[0]);
  ...
  raw.forEach((record, i) => {
    const r = record as Record<string, any>;
    const url = r?.sources?.[0]?.url;
    const sharing = typeof url === 'string' ? licencesByUrl.get(menuIdentity(r.sources[0])) : undefined;
