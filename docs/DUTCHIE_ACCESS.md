...
## Плановый прогон 25.09 — итог: третий исход

Run 36156666957 (обычная скорость, опубликован 19:18 UTC) против медленного
36130424372 того же дня:

| | Медленный | Плановый |
|---|---|---|
| Всего Dutchie | 44 из 101 | 43 из 101 |
| «Сломались 24.09» (окно dutchie.com) | 1 из 30 | 0 из 30 |
| «Стабильные» (Dutchie Plus) | 40 из 41 | 40 из 41 |
| «Не читаются ни разу» | 3 из 26 | 3 из 26 |

По магазинам: 43 прочитаны обоими прогонами, 57 — ни одним, 1 — только
медленным (Emerald Dispensary Carroll Gardens). Скорость на результат не
влияет: те же 30 магазинов с окном dutchie.com закрыты при любой нагрузке.

Вывод — **третий исход**: это защита dutchie.com от автоматических браузеров,
а не наша нагрузка. Что делаем:

- плановый прогон оставляем как есть — Dutchie Plus в нём читается стабильно;
- медленный прогон по расписанию не ставим, workflow `menu-dutchie.yml`
  остаётся только для ручного запуска;
- обходить защиту не будем; путь — договорённость с магазинами (ссылка на
  меню на их собственном сайте или доступ к их API) и честный бот:
  собственное имя в User-Agent, страница о боте с контактом, постоянный адрес
  с обратной DNS-записью, подпись запросов (Web Bot Auth), заявка в Cloudflare
  Verified Bots и письмо в Dutchie;
- у 30 закрытых магазинов на сайте остаётся дата их последнего чтения.
