...
- `robotsAllows` — по `robots.txt` **того хоста, где лежит меню**, для
  `User-agent: *`. Три значения, не два: `true`, `false` и `null`. Если
  `false` — всё равно сдайте запись: нам важно знать, что путь есть, но
  закрыт; сборщик такой адрес не тронет. `null` — когда сам файл прочитать не
  удалось: 403 или 401 на `/robots.txt` (CDN, WAF, прокси) правилом не
  является, и записывать его как запрет магазина нельзя. В этом случае
  обязательно напишите причину в `notes` — валидатор её требует.
